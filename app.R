...
ui <- fluidPage(
  titlePanel(div(
    img(src= "GME.jpg",  height = 130, align = "right"),
...
server <- function(input, output) {
 ...
 observeEvent(input$dir_wd,{
  ...
  observeEvent(input$save,{
    ...
    output$message <- renderText(paste("O metadado do experimento foi salvo em: ", parseDirPath(c(home = '~'), dirwd()))) 
    infile <- parseFilePaths(c(home = '~'), input$file1)
  ...
  observeEvent(input$save2,{
    ...
    writeLines(metadatadata, 
               paste(input$pesquisador,"_",idmeta,"_",Sys.Date(),"_metadata", ".html", sep=""))
    output$message2 <- renderText(paste("O metadado do experimento foi salvo em: ", parseDirPath(c(home = '~'), dirwd()))) 
